...
def entropy(counter):
    total = sum(counter.values())
    return -sum((c / total) * math.log2(c / total) for c in counter.values() if c) + 0.0
...
def seq_shuffled(text, length, seed):
    rng = random.Random(seed)
    pool = list(text[:length])
    rng.shuffle(pool)
    return "".join(pool)
...
V = seq_random_uniform(ALPHABET, seq_len, seed=1)
...
G = seq_shuffled(A, seq_len, seed=2)
D = seq_periodic(top2[0], top2[1], seq_len)
